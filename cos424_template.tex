...
\begin{figure}[ht!]
	...
	\centering
	\includegraphics*[scale=0.55, viewport= -0mm 40mm 500mm 230mm, angle=0]{./figure/stations_events.pdf}
...
\begin{figure}[ht!]
	% Requires \usepackage{graphicx}
	\centering
	\includegraphics*[scale=0.85, viewport= -5mm 0mm 500mm 100mm, angle=0]{./figure/sta_lta.pdf}
...
%		\multicolumn{4}{c|}{Feature selection}  \\
		...
		RF & \textbf{0.0451} & \textbf{0.0388} & \textbf{0.0981} & \textbf{-0.1536} & 1663 \\ 
...
Again, considering the symmetrical nature of input, i.e., given the sign reverted ground motion, it ends up with same magnitude of earthquakes, we did one more step, using the envelope transition, rather than the raw waveform, as the features of input layer. To avoid over-fitting, we include the $\l2$ regularization and a dropout layer.
...
In order to tune the hyper-paramters, we split the train data one step more, keeping 80\% as train data and 20\% as validation data. Table\ref{tab:nn_cv} shows our validation results, using the displacement as the input feature. From the table, we can see that there is really no clear patterns as to set the number of layers and neurons, indicating the complexity and non-linearity of neuron networks. One observation is that  the model performance is not optimal when 300 neurons in each hidden layer are used. Since our input layer is in dimension of 181, 300 neurons might lead to over-fitting.  On the other hand, a small number of neurons might leads to under-fit.
...
\label{fig:svd_right}
\end{subfigure}
\caption{Training and prediction results from SVD. The figure on the left shows the proportion of variances explained by singular values in descending order. The figure on the right shows the probability of future transaction between addresses in the test data.}
\label{fig:combined}
\end{figure}
